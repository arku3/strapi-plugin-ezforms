...
import submitController from "../server/src/controllers/submit-controller";
...
describe("Submit Controller", function () {
      ...
      config: {
        get: vi.fn(function (key) {
          if (key === "plugin::ezforms.captchaProvider.name") {
            ...
            key === "plugin::ezforms.captchaProvider.config.secretKey"
          ) {
            return "secretKey";
  ...
  test("should return captcha error", async function () {
    strapi.plugin().service().validate = vi.fn(function () {
      return {
        valid: false,
        message: "Unable to verify captcha",
        code: 500,
  ...
  test("should send notifications", async function () {
    strapi.config.get = vi.fn(function (key) {
      if (key === "plugin::ezforms.captchaProvider.name") {
        return "none";
