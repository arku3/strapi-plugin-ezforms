...
const hcaptcha = ({ strapi }) => ({
  async validate(token) {
    ...
    let hCaptcha_verify;
    try {
      hCaptcha_verify = await axios__default.default.post(
        url,
        new URLSearchParams({
          secret,
          sitekey,
...
const recaptcha = ({ strapi }) => ({
  async validate(token) {
    ...
    try {
      recaptcha_verify = await axios__default.default.post(url);
    } catch (e) {
      strapi.log.error(e);
      return {
        valid: false,
...
const turnstile = ({ strapi }) => ({
  async validate(token) {
    ...
    try {
      turnstile_verify = await axios__default.default.post(url, {
        secret,
        response: token,
        remoteip
      });
